...
function esebills_enqueue_styles() {
    ...
    if ($post && (has_shortcode($post->post_content, 'esebills_checkout') || has_shortcode($post->post_content, 'esebills_products'))) {
        wp_enqueue_style('esebills-brand', plugin_dir_url(__FILE__) . 'assets/brand.css', array(), '1.2.0');

        $theme_inline = ':root {
  --esebills-theme-primary: var(--wp--preset--color--primary, var(--color-brand-500));
  --esebills-theme-primary-dark: var(--wp--preset--color--primary, var(--color-brand-700));
  --esebills-theme-on-primary: var(--wp--preset--color--base, var(--color-white));
  --esebills-theme-bg: var(--wp--preset--color--base, var(--color-white));
  --esebills-theme-fg: var(--wp--preset--color--contrast, var(--color-ink));
  --esebills-theme-muted: var(--wp--preset--color--secondary, var(--color-gray-500));
  --esebills-theme-border: var(--wp--preset--color--tertiary, var(--color-gray-200));
}
.esebills-checkout {
  background: var(--esebills-theme-bg, var(--color-white));
  border-color: var(--esebills-theme-border, var(--color-gray-200));
}
.esebills-checkout-header {
  background: linear-gradient(135deg, var(--esebills-theme-primary-dark, var(--color-brand-600)), var(--esebills-theme-primary-dark, var(--color-brand-700)));
  color: var(--esebills-theme-on-primary, var(--color-white));
}
.esebills-checkout-header h3,
.esebills-checkout-header p {
  color: var(--esebills-theme-on-primary, var(--color-white));
}
.esebills-notice {
  background: var(--esebills-theme-bg, var(--color-white));
  border-color: var(--esebills-theme-primary, var(--color-brand-500));
  color: var(--esebills-theme-fg, var(--color-ink));
}
.esebills-success {
  background: var(--wp--preset--color--tertiary, var(--color-brand-50));
  border-color: var(--esebills-theme-primary, var(--color-brand-500));
  color: var(--esebills-theme-primary-dark, var(--color-brand-800));
}
.esebills-payment-info {
  background: var(--wp--preset--color--tertiary, var(--color-brand-50));
  border-color: var(--wp--preset--color--tertiary, var(--color-brand-200));
  color: var(--esebills-theme-primary-dark, var(--color-brand-800));
}
.esebills-field label {
  color: var(--esebills-theme-fg, var(--color-gray-700));
}
.esebills-field input,
.esebills-field select,
.esebills-field textarea {
  border-color: var(--esebills-theme-border, var(--color-gray-300));
  color: var(--esebills-theme-fg, var(--color-ink));
  background: var(--esebills-theme-bg, var(--color-white));
}
.esebills-field input:focus,
.esebills-field select:focus,
.esebills-field textarea:focus {
  border-color: var(--esebills-theme-primary, var(--color-brand-500));
  box-shadow: 0 0 0 3px color-mix(in srgb, var(--esebills-theme-primary, var(--color-brand-500)) 15%, transparent);
}
.esebills-hint {
  color: var(--esebills-theme-muted, var(--color-gray-500));
}
.esebills-btn {
  background: linear-gradient(135deg, var(--esebills-theme-primary, var(--color-brand-500)), var(--esebills-theme-primary-dark, var(--color-brand-600)));
  color: var(--esebills-theme-on-primary, var(--color-white));
}
.esebills-back-link {
  color: var(--esebills-theme-primary, var(--color-brand-600));
}
.esebills-back-link:hover {
  color: var(--esebills-theme-primary-dark, var(--color-brand-700));
}
.esebills-fulfilment {
  background: var(--wp--preset--color--tertiary, var(--color-brand-50));
  border-left-color: var(--esebills-theme-primary, var(--color-brand-400));
  color: var(--esebills-theme-primary-dark, var(--color-brand-800));
}
.esebills-category-header {
  background: linear-gradient(135deg, var(--esebills-theme-primary-dark, var(--color-brand-600)), var(--esebills-theme-primary-dark, var(--color-brand-700)));
}
.esebills-category-header h3,
.esebills-category-count {
  color: var(--esebills-theme-on-primary, var(--color-white));
}
.esebills-category-items {
  background: var(--esebills-theme-border, var(--color-gray-200));
  border-color: var(--esebills-theme-border, var(--color-gray-200));
}
.esebills-product-card {
  background: var(--esebills-theme-bg, var(--color-white));
}
.esebills-product-card:hover {
  background: var(--wp--preset--color--tertiary, var(--color-brand-50));
  box-shadow: inset 3px 0 0 var(--esebills-theme-primary, var(--color-brand-500));
}
.esebills-product-card-body h4 {
  color: var(--esebills-theme-fg, var(--color-ink));
}
.esebills-product-card-body p {
  color: var(--esebills-theme-muted, var(--color-gray-500));
}
.esebills-product-price {
  color: var(--esebills-theme-primary-dark, var(--color-brand-700));
}
.esebills-product-select {
  color: var(--esebills-theme-primary, var(--color-brand-500));
}
.esebills-product-card:hover .esebills-product-select {
  color: var(--esebills-theme-primary-dark, var(--color-brand-700));
}
@media (prefers-color-scheme: dark) {
  :root:not([data-theme="light"]) {
    --esebills-theme-bg: var(--wp--preset--color--base, #1a1c20);
    --esebills-theme-fg: var(--wp--preset--color--contrast, #f5f5f5);
    --esebills-theme-muted: var(--wp--preset--color--secondary, #9ca3af);
    --esebills-theme-border: var(--wp--preset--color--tertiary, #374151);
  }
}
html[data-theme="dark"] {
  ...
  --esebills-theme-border: var(--wp--preset--color--tertiary, #374151);
        ...
        wp_add_inline_style('esebills-brand', $theme_inline);
    }
}
...
function esebills_products_shortcode($atts) {
    ...
    if (empty($checkout_url) && isset($_SERVER['REQUEST_URI'])) {
        $request_uri = esc_url_raw(wp_unslash($_SERVER['REQUEST_URI']));
        if (false !== strpos($request_uri, 'product_code=')) {
